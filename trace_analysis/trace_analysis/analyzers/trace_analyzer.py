...
from ..models.span_info import SpanInfo
...
class TraceAnalyzer:
    def __init__(self):
        """Initialize the TraceAnalyzer with required components."""
        self.all_traces = {}
        self.all_traces_info = {}
        self.abnormal_traces = []
        self.turning_factor = 1.3
    ...
    def write_analysis_results(self, output_file: str) -> None:
        """Write all analysis results to a single file."""
        with open(output_file, 'w') as f:
            # Write critical paths analysis
            groups = self.group_similar_critical_paths()
            self._write_critical_paths_analysis(f, groups)
            
            # Write matrix analysis
            self._write_matrix_analysis(f)
            
            # Write ranking analysis
            self._write_ranking_analysis(f)
            
            # Write node ranks
            f.write("\nNODE RANKS\n")
            f.write("=" * 50 + "\n\n")
            ranks = self.calculate_node_ranks()
            for node, rank in list(ranks.items())[:10]:
                f.write(f"{node}: {rank:.6f}\n")
            
            # Write PageRank analysis
            f.write("\n\nPAGERANK ANALYSIS\n")
            f.write("=" * 50 + "\n\n")
            ranks = self.calculate_personalized_pagerank()
            f.write("Top 10 Ranked Nodes:\n")
            for node, rank in list(ranks.items())[:10]:
                f.write(f"{node}: {rank:.6f}\n")
            
            # Write PageRank with preference for abnormal traces
            abnormal_preference = {}
            for trace in self.abnormal_traces:
                abnormal_preference[trace['trace_name']] = 1.0
            
            ranks_abnormal = self.calculate_personalized_pagerank(
            ...
            # Write operation coverage analysis
            self._write_coverage_analysis(f)
            
            # Write abnormal coverage ranking
            ...
            # Write lowest level operations
            f.write("\n\nLOWEST LEVEL OPERATIONS\n")
            f.write("=" * 50 + "\n\n")
            lowest_level_results = self.find_lowest_level_operations()
    ...
    def calculate_node_ranks(self) -> Dict[str, float]:
        """Calculate ranks for all nodes in the graph."""
        operations = set()
        traces = set(self.all_traces_info.keys())
        ...
        matrix_data = self.matrix_analyzer.create_partitioned_matrix(
            operations, traces, self.all_traces_info
        )
        
        ranks = self.ranking_analyzer.calculate_pagerank(matrix_data['matrix'])
        node_list = list(operations) + list(traces)
        return {node: rank for node, rank in zip(node_list, ranks)}

    def calculate_personalized_pagerank(self, 
                                    preference_vector: Optional[Dict[str, float]] = None,
        ...
        # Convert preference vector to numpy array if provided
        if preference_vector is not None:
            node_list = list(operations) + list(traces)
            pref_array = np.zeros(len(node_list))
            for node, value in preference_vector.items():
                if node in matrix_data['node_to_index']:
                    pref_array[matrix_data['node_to_index'][node]] = value
            preference_vector = pref_array / (np.sum(pref_array) or 1.0)
        
        ranks = self.ranking_analyzer.calculate_pagerank(
            matrix_data['matrix'],
    ...
    def _write_critical_paths_analysis(self, f: Any, groups: Dict) -> None:
        """Write critical paths analysis section."""
        f.write("CRITICAL PATHS ANALYSIS\n")
        f.write("=" * 50 + "\n\n")
        
        for group_id, (signature, group_data) in enumerate(groups.items(), 1):
            f.write(f"Group {group_id}:\n")
            f.write(f"Traces: {', '.join(group_data['traces'])}\n")
            f.write(f"Expected Latency: {group_data['expected_latency']:.2f}ms\n")
            f.write("Actual Latencies:\n")
            
            for trace_name, latency in group_data['actual_latencies'].items():
                status = "ABNORMAL" if latency > group_data['expected_latency'] else "normal"
                f.write(f"  {trace_name}: {latency:.2f}ms [{status}]\n")
            
            f.write("\n")

    def _write_matrix_analysis(self, f: Any) -> None:
        """Write matrix analysis section."""
        f.write("\nMATRIX ANALYSIS\n")
        f.write("=" * 50 + "\n\n")
        
        # Get operations and traces
        operations = set()
        traces = set(self.all_traces_info.keys())
        ...
        self.matrix_analyzer.create_partitioned_matrix(
            operations, traces, self.all_traces_info, output_file=f
        )

    def _write_ranking_analysis(self, f: Any) -> None:
        """Write ranking analysis section."""
        f.write("\nRANKING ANALYSIS\n")
        f.write("=" * 50 + "\n\n")
        
        # Calculate and write PageRank scores
        operations = set()
        traces = set(self.all_traces_info.keys())
        ...
        f.write("PageRank Scores:\n")
        node_list = list(operations) + list(traces)
        for i, rank in enumerate(ranks):
            if i < len(node_list):
                f.write(f"{node_list[i]}: {rank:.6f}\n")

    def _write_coverage_analysis(self, f: Any) -> None:
        """Write operation coverage analysis section."""
        f.write("\nOPERATION COVERAGE ANALYSIS\n")
        f.write("=" * 50 + "\n\n")
        
        coverage_scores = self.calculate_operation_coverage_scores()
        for operation, scores in sorted(
            coverage_scores.items(), 
            key=lambda x: x[1]['Oef'], 
            reverse=True
        ):
            ...
            stats = scores['coverage_stats']
            f.write(f"Abnormal Traces: {stats['abnormal_traces_covered']}/")
            f.write(f"{stats['total_abnormal_traces']}\n")
            f.write(f"Normal Traces: {stats['normal_traces_covered']}/")
            f.write(f"{stats['total_normal_traces']}\n")
